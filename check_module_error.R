...
#----------------------------------Visualization-------------------------------------------------------------
### overall dynamics 
plot(sim, type = "epi",mean.line=TRUE,
     qnts = 0.5,legend=TRUE,main='100sims with i0=1&tp=0.99(qnts=0.5)') ## qnts= quantile range set to 0.5(show 50% data)



plot(sim,type = "epi", y = c("se.flow", "ei.flow", 'ea.flow','ar.flow',"ir.flow"), 
     main='flow between boxs(qnts=0.5)',
     popfrac=FALSE,
     qnts= 0.5,legend = TRUE) ## flow between box

df <- as.data.frame(sim) 


df_transmat <- data.frame()
for (i in 1:100){
  try({df <- get_transmat(sim, sim =i)})
  #### previously, you can still get an empty df if no transmat, now it will get error 
  df$sim <- i
  df_transmat <- rbind(df_transmat,df)
  
}


seed21 <- df_transmat[df_transmat$inf==21,]
View(seed21)
